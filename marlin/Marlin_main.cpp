...
#include "feedback_system.h"	// feedback_system @ ajienikicio 20140213
#include "Adafruit_NeoPixel.h"	// @ ajienikicio
#include "laser.h"				// laser @ ajienikicio 20140523
...
{	
	...
	SERIAL_ECHO(++y_count);
	SERIAL_ECHO(" y=");
	SERIAL_ECHOLN(y_step);
	
	plan_buffer_line(0.00, y_step-5, 0.00, 0.00, 50, 0);
	plan_buffer_line(0.00, y_step-1, 0.00, 0.00, 50, 0);
	y_step-=1;
}
...
//{
	...
	////PCICR &= ~(1 << PCIE2);
	//y_step=-700.00;
	//plan_buffer_line(0.00, y_step, 0.00, 0.00, 50, 0);
//}
...
{	
  // turn led strip	on
  #if USE_LED_STRIP
  strip.setPin(6);	// PD3 for Gen6.d, PA6 for SmithBoard
  strip.begin();
  strip.show(); // Initialize all pixels to 'off'  
  strip.colorWipe(strip.Color(255, 255, 255), 0); // Green	
  #endif
  
  laser_init();
  
  setup_killpin();	// not for gen6
  setup_powerhold();	// not for gen6
  ...
  #if USE_THERMISTOR
  tp_init();    // Initialize temperature loop, this enables TIMER0B interrupt	@ ajienikicio 20140210
  #endif
  plan_init();  // Initialize planner;
  watchdog_init();
  st_init();    // Initialize stepper, this enables TIMER1A (and TIMER0A, advanced) interrupts	@ ajienikicio 20140210
  setup_photpin();	// not for gen6
  servo_init();		// not fot gen6
...
{    
  if (y_count == 168)
  {
	plan_buffer_line(0.00, -650.00, 0.00, 0.00, 50, 0);
	PCICR &= ~(1 << PCIE2);	
  }
  
  //colorWipe(strip.Color(255, 255, 255), 0); // Green	
  //rainbow(20);
  		
  //SERIAL_ECHO(step_y_events_position + step_y_events_position); SERIAL_ECHO(" ");  
  //SERIAL_ECHO(step_y_events_position); SERIAL_ECHO("\n");
  //SERIAL_ECHO(en_counter); SERIAL_ECHO("\n");
  //SERIAL_ECHO(en_counter2); SERIAL_ECHO("\n");  
		  
  //SERIAL_ECHO(isr_period); SERIAL_ECHO("\n");
  //SERIAL_ECHOLN(en_counter2);
...
void process_commands()
{
  ...
  else if(code_seen('M'))
  {
    ...
    case 117: // M117 display message
      ...
      break;
	// Lasers for 3D Scanner @ ajienikicio 20140523
	case 111: // M111 turn on laser
	  if (code_seen('L')) laserOn(LASER_LEFT_PIN);			// M111 L
	  else if (code_seen('R')) laserOn(LASER_RIGHT_PIN);	// M111 R
	  break;
	case 112: // M112 turn off laser
	  if (code_seen('L')) laserOff(LASER_LEFT_PIN);			// M112 L
	  else if (code_seen('R')) laserOff(LASER_RIGHT_PIN);	// M112 R	
	  break;	  
    case 114: // M114
